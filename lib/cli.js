...
module.exports = {
  ...
  /*
  |-----------------------------------------------------------------------------
  | Register configurations                https://docs.codolog.com/larastrator
  |-----------------------------------------------------------------------------
  |
  |
  */
...
let shadows = `
  shadows: {
    'input': '0 0 0 3px rgba(201, 210, 218, 0.5)',
...
_commander2.default.command('init [filename]').usage('[options] [filename]').option('-c, --config [path]', 'Path to tailwind config file').action((filename = 'larastrator.js', options) => {
  ...
  let lsOutput = _fsExtra2.default.readFileSync(_path2.default.resolve(__dirname, '../defaultConfig.stub.js'), 'utf8');
  twOutput = twOutput.replace('let colors = {', colors);
  twOutput = twOutput.replace('module.exports = {', lsOutput);
  twOutput = twOutput.replace('// module.exports = {', 'module.exports = {');
  twOutput = twOutput.replace('shadows: {', shadows);
  _fsExtra2.default.outputFileSync(lsDestination, twOutput.replace('module.exports = {', init));

  console.warn(`Generated Larastrator config: ${filename}`);
